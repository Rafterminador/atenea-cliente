...
const login = async (auth, email, password) => {
    ...
    response = await signInWithEmailAndPassword(auth, email, password)
        .then((userCredential) => {
            ...
            response = { uid: response.uid, email: response.email, name: response.displayName, role: roleObject.rol, number: response.phoneNumber === null ? "" : response.phoneNumber.slice(4) || "", password: password }
            const auth = getAuth();
